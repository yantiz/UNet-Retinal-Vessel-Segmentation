...
from pre_processing import my_PreProc_RGB, my_PreProc_gray

config = ConfigParser.RawConfigParser()
config.read('configuration.txt')
path_name = config.get('experiment name', 'name')
...
def extract_random(full_imgs,full_masks, patch_h,patch_w, N_patches, inside=True):
    if (N_patches%full_imgs.shape[0] != 0):
        print "N_patches: plase enter a multiple of the training set size"
        exit()
    assert (len(full_imgs.shape)==4 and len(full_masks.shape)==4)  #4D arrays
    assert (full_imgs.shape[1]==1 or full_imgs.shape[1]==3)  #check the channel is 1 or 3
    assert (full_masks.shape[1]==3)   #masks have three classes
...
def pred_only_FOV(data_imgs,data_masks,original_imgs_border_masks):
    ...
    assert (data_imgs.shape[1]==1 or data_imgs.shape[1]==3)  #check the channel is 1 or 3
    assert (data_masks.shape[1]==3)  #check the channel is 3
...
def pred_normal(data_imgs,data_masks):
    assert (len(data_imgs.shape)==4 and len(data_masks.shape)==4)  #4D arrays
    assert (data_imgs.shape[0]==data_masks.shape[0])
    assert (data_imgs.shape[2]==data_masks.shape[2])
    assert (data_imgs.shape[3]==data_masks.shape[3])
